...
/**
 * Created by suchismita.p on 9/23/2016.
 */
...
public class ContactOperation {
    ...
    void addPhoto(ArrayList<ContentProviderOperation> ops){
        ...
        cv.put(ContactsContract.Contacts.Photo.PHOTO,ImageContact(R.drawable.ic_launcher));
        cv.put(ContactsContract.Contacts.Photo.MIMETYPE, ContactsContract.CommonDataKinds.Photo.CONTENT_ITEM_TYPE);
        ContentProviderOperation.Builder insertOp = createInsertForContact(-1, cv);
    ...
    private ContentProviderOperation.Builder createInsertForContact(long rawContactId, ContentValues cv) {
        ContentProviderOperation.Builder insertOp = ContentProviderOperation.newInsert(ContactsContract.Data.CONTENT_URI).withValues(cv);
        if (rawContactId == -1) {
            insertOp.withValueBackReference(ContactsContract.Data.RAW_CONTACT_ID, 0);
        } else {
            insertOp.withValue(ContactsContract.Contacts.Data.RAW_CONTACT_ID, rawContactId);
        }
        return insertOp;
    }
